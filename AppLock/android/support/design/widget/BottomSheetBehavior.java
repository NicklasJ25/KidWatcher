...
public class BottomSheetBehavior<V extends View> extends Behavior<V> {
    ...
    private final Callback mDragCallback = new C00302();
    boolean mHideable;
    ...
    protected static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C00311());
        final int state;

        static class C00311 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00311() {
            }
    ...
    public BottomSheetBehavior(Context context, AttributeSet attributeSet) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0006R.styleable.BottomSheetBehavior_Layout);
        TypedValue peekValue = obtainStyledAttributes.peekValue(C0006R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight);
        if (peekValue == null || peekValue.data != -1) {
            setPeekHeight(obtainStyledAttributes.getDimensionPixelSize(C0006R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight, -1));
        } else {
            setPeekHeight(peekValue.data);
        }
        setHideable(obtainStyledAttributes.getBoolean(C0006R.styleable.BottomSheetBehavior_Layout_behavior_hideable, false));
        setSkipCollapsed(obtainStyledAttributes.getBoolean(C0006R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed, false));
    ...
    public boolean onLayoutChild(CoordinatorLayout coordinatorLayout, V v, int i) {
        ...
        if (this.mPeekHeightAuto) {
            if (this.mPeekHeightMin == 0) {
                this.mPeekHeightMin = coordinatorLayout.getResources().getDimensionPixelSize(C0006R.dimen.design_bottom_sheet_peek_height_min);
            }
    ...
    boolean shouldHide(View view, float f) {
        return this.mSkipCollapsed ? true : view.getTop() < this.mMaxOffset ? false : Math.abs((((float) view.getTop()) + (HIDE_FRICTION * f)) - ((float) this.mMaxOffset)) / ((float) this.mPeekHeight) > HIDE_THRESHOLD;
    }
